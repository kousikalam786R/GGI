class Game {
    ...
    async intro() {
        ...
        try {
            const response = await fetch("https://testapi.galaxygaminginu.com/coloron/create_user", {
                method: "POST",
                headers: {
    ...
    start() {
        ...
        (async() => {
            try {
                const body = { score };
                const response = await fetch("https://testapi.galaxygaminginu.com/coloron/update_user", {
                    method: "POST",
                    headers: {
                        "Content-type": "application/json",
                        account: localStorage.account
        ...
        $('#sticks .stick').each(function() {
            if ($(this).offset().left < ballPos && $(this).offset().left > ballPos - stickWidth) {
                ...
                if (Color.getColorFromClass($(this)) == Color.getColorFromClass('#ball')) {
                    ...
                    TweenMax.fromTo('#coins', 0.5, { scale: 1.5 }, { scale: 1, ease: Elastic.easeOut.config(1.5, 0.5) });
                } else {
