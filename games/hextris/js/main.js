const { post } = require("../../../server/routes/hextris");
...
async function creatUser() {
    try {
        const response = await fetch("https://testapi.galaxygaminginu.com/hextris/create_user", {
            method: "POST",
            headers: {
        ...
        $("#coinText").html(parseRes.coins);
        $("#currentHighScore").html(parseRes.highScore);
    } catch (err) {
    ...
    switch (gameState) {
        case 1:
            ...
            if (checkGameOver() && !importing) {
                ...
                saveState = JSONfn.parse(saveState);
                gameState = 2;
                //As soon as game is over update the data
                updateData();
                setTimeout(function() {
                    enableRestart();
                }, 150);
...
async function updateData() {
    try {
        const body = { score }
        const response = await fetch("https://testapi.galaxygaminginu.com/hextris/update_user", {
            method: "POST",
            headers: {
                "Content-type": "application/json",
                account: localStorage.account
